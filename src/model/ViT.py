...
class ViT(BaseModel):
    def __init__(self, cfg):
        ...
        self.cls_flag = cfg.vit_cls_flag
        self.dropout = nn.Dropout(cfg.attention_dropout)
        self.blocks = nn.ModuleList([ViTBlock(cfg) for _ in range(cfg.num_hidden_layers)])
    ...
    def load_weights(self, weight_dict: dict):
        ...
        for hf_key, our_key in mapping.items():
            if hf_key in weight_dict.keys() and our_key in sd:
                ...
                if hf_key not in weight_dict.keys():
        ...
        for i in range(self.cfg.num_hidden_layers):
            ...
            q_bias = weight_dict[f'vision_model.encoder.layers.{i}.self_attn.q_proj.bias']
            k_bias = weight_dict[f'vision_model.encoder.layers.{i}.self_attn.k_proj.bias']
            v_bias = weight_dict[f'vision_model.encoder.layers.{i}.self_attn.v_proj.bias']
